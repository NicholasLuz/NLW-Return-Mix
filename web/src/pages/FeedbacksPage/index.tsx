...
type IdsFirebaseFeedbacks = Record<string, {
  author: {
    avatar: string;
    id: string;
    name: string;
...
export function FeedbacksPage() {
  ...
  const [allFeedbacks, setAllFeedbacks] = useState<AllFeedbacksProps[]>([]);
  ...
  useEffect(() => {
    feedbackRef.on('value', room => {
      const firebaseIds: IdsFirebaseFeedbacks= room.val();
      
      const parsedFeedbacks = Object.entries(firebaseIds).map(([key, value]) => {
        return {
            authorId: key,
            authorAvatar: value.author.avatar,
            authorName: value.author.name,
            feedbackCards: value.feedbackCard,
        };
      })
      setAllFeedbacks(parsedFeedbacks);
    })
  ...
  // const feedbackBug = idsFirebase.filter(feedback => feedback. === "BUG");
  // const feedbackIdea = idsFirebase.filter(feedback => feedback.type === "IDEA");
  // const feedbackOther = idsFirebase.filter(feedback => feedback.type === "OTHER");
        ...
        <div className="wrapper">
            <header>
                <div className="content">
                    <a className="logo" onClick={handleReturnToHome}>
        ...
        <div className="wrapper">
            <main>
                <div className="page-title">
                    <h1>Feedbacks de {user?.name}</h1>
                    {/* { feedbacks.length > 0  &&  <span> { feedbacks.length } feedback(s)</span> } */}
                    <p>
                        <button onClick={handleReturnToHome}>Voltar para Home</button>
                    </p>
                </div>

                <div className="feedback-list">
                    <div className="bugs">BUGS</div>
                    {/* <div className="feedbacks-bug">
                        {feedbackBug.map(feedback => {
                            return(
                                <FeedbackCard
                                    key={feedback.id}
                    ...
                    <div className="ideas">IDEIAS</div>
                    {/* <div className="feedbacks-idea">
                        {feedbackIdea.map(feedback => {
                            return(
                                <FeedbackCard
                                    key={feedback.id}
                                    comment={feedback.comment}
                    ...
                    <div className="others">OUTROS</div>
                    {/* <div className="feedbacks-other">
                        {feedbackOther.map(feedback => {
                            return(
                                <FeedbackCard
                                    key={feedback.id}
                                    comment={feedback.comment}
