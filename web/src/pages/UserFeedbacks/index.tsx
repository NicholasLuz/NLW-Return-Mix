...
export function UserFeedbacks() {
  ...
  const code = user?.id;
  const { feedbacks } = useRoom(code!);

  const feedbackBug = feedbacks.filter(feedback => feedback.type === "BUG");
  const feedbackIdea = feedbacks.filter(feedback => feedback.type === "IDEA");
  const feedbackOther = feedbacks.filter(feedback => feedback.type === "OTHER");
